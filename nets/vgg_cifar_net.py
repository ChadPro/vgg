...
def vgg_conv(net_input, filter_size, strides, padding, scope, trainable=True):
    with tf.variable_scope(scope):
        ...
        net = ACTIVATION(tf.nn.bias_add(net, conv_biases))
...
def vgg_fc(net_input,fc_size,train,regularizer,scope):
    with tf.variable_scope(scope):
        ...
        fc = ACTIVATION(tf.matmul(net_input, fc_weights) + fc_biases)
        if train == "train":
...
def vgg_net(inputs, \
    ...
    bgr = tf.concat(axis=3, values=[blue - CIFAR10_MEAN[0], green - CIFAR10_MEAN[1], red - CIFAR10_MEAN[2], ])
    ...
    with tf.variable_scope(scope):
        ...
        with tf.variable_scope('part_6'):
            net = vgg_fc(net_reshaped, fc_size=[nodes_num,1024], train=train, regularizer=regularizer, scope="fc_1")
            net = vgg_fc(net, fc_size=[1024,1024], train=train, regularizer=regularizer, scope="fc_2")
            net = vgg_logit(net, fc_size=[1024,num_classes], regularizer=regularizer, scope="fc_3")

    return net, variables_restore