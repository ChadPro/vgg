...
tf.app.flags.DEFINE_integer('image_size', IMAGE_SIZE, 'Data image size.')
...
def train():
    ...
    x = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.image_size, FLAGS.image_size, 3], name='net_x_input')
    ...
    y, variables_restore = vgg_net.vgg_net(x, num_classes=FLAGS.num_classes, is_training=True, train='train', fine_tune=FLAGS.fine_tune, regularizer=regularizer)
    ...
    total_loss = cross_entropy_mean + tf.add_n(tf.get_collection('regularizer_losses'))
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(FLAGS.total_steps):
            ...
            if i % 30 == 0:
                ...
                if len(FLAGS.val_data_path) > 1:
                    x_input_val, y_input_val = sess.run([input_X_val, input_Y_val])
                else:
                    x_input_val, y_input_val = input_X, input_Y
